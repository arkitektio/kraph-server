...
class ScatterPlotInput:
    ...
    id_column: str = strawberry.field(description="The column to use for the ID of the points")
    x_column: str = strawberry.field(description="The column to use for the x-axis")
    x_id_column: str | None = strawberry.field(default=None, description="The column to use for the x-axis ID (node, or edge)")
    y_column: str = strawberry.field(description="The column to use for the y-axis")
    y_id_column: str | None = strawberry.field(default=None, description="The column to use for the y-axis ID (node, or edge)")
    size_column: str | None = strawberry.field(default=None, description="The column to use for the size of the points")
    color_column: str | None = strawberry.field(default=None, description="The column to use for the color of the points")
    shape_column: str | None = strawberry.field(default=None, description="The column to use for the shape of the points")
    test_against: strawberry.ID | None = strawberry.field(default=None, description="The graph to test against")
...
def check_id_column(columns: list[inputs.ColumnInput], name: str) -> str:
...
def update_scatter_plot(info: Info, input: UpdateScatterPlotInput) -> types.ScatterPlot:
    splot = models.ScatterPlot.objects.get(id=input.id)

    columns = splot.query.input_columns

    if input.name:
        splot.name = input.name

    if input.id_column:
        splot.id_column = check_id_column(columns, input.id_column)
