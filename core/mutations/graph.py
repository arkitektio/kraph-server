...
def update_graph(info: Info, input: UpdateGraphInput) -> types.Graph:
    ...
    if input.nodes is not None and input.edges is not None:
        ...
        structure_nodes = [node for node in input.nodes if node.kind == enums.OntologyNodeKind.STRUCTURE]
        entity_nodes = [node for node in input.nodes if node.kind == enums.OntologyNodeKind.ENTITY]
        metric_nodes = [node for node in input.nodes if node.kind == enums.OntologyNodeKind.METRIC]
        ...
        for metric_node in metric_nodes:
            assert metric_nodes.metric_kind is not None, "Measurement edges must have a measurement kind"
        ...
        for edge in measurement_edges:
            ...
            medge, created = models.MeasurementCategory.objects.update_or_create(
                ...
                ontology=item,
                left=models.StructureCategory.objects.get(age_name=edge.source, ontology=item),
                right=models.EntityCategory.objects.get(age_name=edge.target, ontology=item),
                defaults=dict(
        ...
        for edge in relation_edges:
            ...
            redge, created = models.RelationCategory.objects.update_or_create(
                ...
                defaults=dict(
                    description=edge.description or "",
                    purl=edge.purl or "",
                    left=models.EntityCategory.objects.get(age_name=edge.source, ontology=item),
                    right=models.EntityCategory.objects.get(age_name=edge.target, ontology=item),
                ),
            )
