...
def knowledge_graph(id: strawberry.ID) -> KnowledgeGraph:
    ...
    for entity_kind in entity_kinds:
        ...
        for metric in models.EntityMetric.objects.filter(kind=entity_kind):
            node.metrics.append(
                EntityKindNodeMetric(data_kind=metric.data_kind, kind=metric.kind.label)
            )

        nodes.append(node)

    for entity_relation_kind in models.EntityRelationKind.objects.filter(
        left_kind__in=entity_kinds, right_kind__in=entity_kinds
    ):
        edge = EntityKindRelationEdge(
            id=entity_relation_kind.id,
            label=entity_relation_kind.kind.label,
            source=entity_relation_kind.left_kind.id,
            target=entity_relation_kind.right_kind.id,
            metrics=[],
        )

        for metric in models.RelationMetric.objects.filter(
            kind=entity_relation_kind.kind
        ):
            edge.metrics.append(
                EntityKindNodeMetric(data_kind=metric.data_kind, kind=metric.kind.label)
            )
