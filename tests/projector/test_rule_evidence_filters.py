...
def _definition(*, rule_evidence: dict | None = None, category_definition: models.CategoryDefinitionInput | None = None) -> models.GraphDefinitionInput:
...
def test_rule_evidence_applies_on_an_unscoped_category(transactional_db, table_projector, authenticated_context) -> None:
    ...
    graph = _graph(_definition(rule_evidence=R.evidence(R.rule(R.via("good-tool")))), table_projector, authenticated_context, "rule-unscoped")
...
def test_rule_evidence_replaces_the_default(transactional_db, table_projector, authenticated_context) -> None:
    ...
    category_definition = models.CategoryDefinitionInput.model_validate(R.definition(R.rule(R.word("Probe"), R.by("peter"))))
    graph = _graph(
        _definition(category_definition=category_definition, rule_evidence=R.evidence(R.rule(R.via("good-tool")))),
        table_projector,
                    ...
                    property_definitions=[
                        models.PropertyDefinitionInput(
                            ...
                            rule=models.DerivationRuleInput(
                                ...
                                evidence=R.evidence(R.rule(R.by("pipeline"))),
                            ),
                        )
                    ],
                )
...
def test_evidence_without_a_source_is_refused() -> None:
    with pytest.raises(ValidationError, match="source_node"):
        models.DerivationRuleInput(evidence=R.evidence(R.rule(R.by("x"))))
...
def test_a_rule_evidence_change_moves_the_definition_hash() -> None:
    ...
    plain = _definition(rule_evidence=R.evidence(R.rule(R.via("a"))))
    other = _definition(rule_evidence=R.evidence(R.rule(R.via("b"))))
...
@pytest.mark.django_db(transaction=True)
def test_rule_evidence_unless_subtracts(transactional_db, table_projector, authenticated_context) -> None:
    evidence = R.evidence(R.rule(R.not_by("bot"), unless=[[R.via("old-pipeline")]]))
    graph = _graph(_definition(rule_evidence=evidence), table_projector, authenticated_context, "rule-unless")
    org = graph.organization
    ref = claims.mint(org, "Probe", "anyone")
    claims.measure(org, ref, obj="r1", key="vector_length", value=10.0, subject="peter", app_id="curation-ui")
    claims.measure(org, ref, obj="r2", key="vector_length", value=500.0, subject="peter", app_id="old-pipeline")
    claims.measure(org, ref, obj="r3", key="vector_length", value=900.0, subject="bot", app_id="curation-ui")
    properties = _fold(graph, table_projector, ref)
    assert properties.get("avg_length") == pytest.approx(10.0), f"the exception blocks the old pipeline, the NOT_IN the bot: {properties}"


@pytest.mark.django_db(transaction=True)
def test_key_in_rule_evidence(transactional_db, table_projector, authenticated_context) -> None:
    """The property reads `vector_length`; the evidence says app B counts only
    for `area`, so B's vector_length never folds while A's does."""
    evidence = R.evidence(R.rule(R.via("app-a")), R.rule(R.via("app-b"), R.key("area")))
    graph = _graph(_definition(rule_evidence=evidence), table_projector, authenticated_context, "rule-key")
    org = graph.organization
    ref = claims.mint(org, "Probe", "anyone")
    claims.measure(org, ref, obj="r1", key="vector_length", value=10.0, subject="pipeline", app_id="app-a")
    claims.measure(org, ref, obj="r2", key="vector_length", value=900.0, subject="pipeline", app_id="app-b")
    properties = _fold(graph, table_projector, ref)
    assert properties.get("avg_length") == pytest.approx(10.0), f"app-b is trusted for area only: {properties}"


@pytest.mark.django_db(transaction=True)
def test_key_in_a_measurement_only_category_rule(transactional_db, table_projector, authenticated_context) -> None:
    """The category default: app A for vector_length, app B for area. Two
    properties without their own evidence, each folds only its app."""
    category_definition = models.CategoryDefinitionInput.model_validate(
        R.definition(
            R.rule(R.word("Probe"), R.not_kind("MEASUREMENT")),
            R.rule(R.of_kind("MEASUREMENT"), R.via("app-a"), R.key("vector_length")),
            R.rule(R.of_kind("MEASUREMENT"), R.via("app-b"), R.key("area")),
        )
    )
    definition = models.GraphDefinitionInput(
        system_version="1.0.0",
            ...
            entities=[
                models.EntityDefinitionInput(
                    ...
                    property_definitions=[
                        ...
                        models.PropertyDefinitionInput(key="avg_area", type=models.PropertyType.FLOAT, derivation=models.DerivationType.ROLLUP, rule=models.DerivationRuleInput(source_node="ROI", key="area", aggregation=models.AggregationFunction.MEAN)),
                    ],
                )
            ]
        ),
    )
    graph = _graph(definition, table_projector, authenticated_context, "category-key")
    org = graph.organization
    ref = claims.mint(org, "Probe", "anyone")
    claims.measure(org, ref, obj="r1", key="vector_length", value=10.0, subject="pipeline", app_id="app-a")
    claims.measure(org, ref, obj="r2", key="vector_length", value=900.0, subject="pipeline", app_id="app-b")
    claims.measure(org, ref, obj="r3", key="area", value=5.0, subject="pipeline", app_id="app-b")
    claims.measure(org, ref, obj="r4", key="area", value=700.0, subject="pipeline", app_id="app-a")
    properties = _fold(graph, table_projector, ref)
    assert properties.get("avg_length") == pytest.approx(10.0), f"vector_length comes from app-a only: {properties}"
    assert properties.get("avg_area") == pytest.approx(5.0), f"area comes from app-b only: {properties}"
