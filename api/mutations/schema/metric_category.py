...
def create_metric_category(
    info: Info,
    input: inputs.CreateMetricDefinitionInput,
    ...
    ent = models.MetricCategory.objects.create_from_metric_definition(
        graph,
        definition=model,
    )

    return cast(types.MetricCategory, ent)


def update_metric_category(info: Info, input: inputs.UpdateMetricDefinitionInput) -> types.MetricCategory:
    """GraphQL mutation wrapper for updating metric categories."""
    model = input.to_pydantic()  # Validate input with Pydantic models

    item = models.MetricCategory.objects.get(id=model.id)
...
def delete_metric_category(
    info: Info,
    input: inputs.DeleteMetricDefinitionInput,
) -> strawberry.ID:
    model = input.to_pydantic()  # Validate input with Pydantic models
    item = models.MetricCategory.objects.get(id=model.id)
    item.delete()
