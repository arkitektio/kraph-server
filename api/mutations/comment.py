...
def retract_comment(
    ...
    return types.AssertedComment(_value=controller.retract_comment(comment_id=str(model.id), info=info))
...
def attest_comment(
    ...
    controller = context.get_controller()
    model = input.to_pydantic()
    return types.AssertedComment(_value=controller.attest_comment(comment_id=str(model.id), info=info))
