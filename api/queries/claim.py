...
`node(id:)` and `relation(id:)` answer with a *drawing* — how some view holds the
thing, derived properties and all. These two answer with the recorded statement:
...
def instance(info: Info, id: scalars.GraphID) -> types.Instance:
    ...
    Answers for a claim no view draws, which is exactly what distinguishes it from
    `node(id:)`: that one reads a drawing and falls back to the log, this one never
    consults a projection at all.
    """
