"""Structure relation query resolvers."""
...
def structure_relations(
    ...
    """Fetch structure relations for a given category with optional filters, ordering, and pagination."""
    controller = context.get_controller()

    category = models.StructureRelationCategory.objects.filter(id=structure_relation_category_id).first()
    if category is None:
        raise ValueError(f"Structure relation category {structure_relation_category_id} not found")
    ...
    return [types.StructureRelation(_value=_to_retrieved_edge(str(graph.age_name), row)) for row in result]
