"""Output participation edge query resolvers."""
...
def output_participation(info: Info, id: scalars.GraphID) -> types.OutputParticipation:
    """Fetch a specific output participation edge by composite graph ID."""
    ...
    result = controller.engine.execute(
        ...
        WHERE id(r) = $rid AND (source:NaturalEvent OR source:ProtocolEvent)
...
def output_participations(
    ...
    """Fetch output participation edges with optional filters, ordering, and pagination."""
    controller = context.get_controller()
    ...
    result = controller.engine.execute(
        ...
        f"""
        MATCH (source)-[r]->(target:Entity)
        WHERE (source:NaturalEvent OR source:ProtocolEvent) AND {" AND ".join(where_clauses)}
        RETURN r, type(r) as label, id(r) as id, id(startNode(r)) as left_id, id(endNode(r)) as right_id
    ...
    return [types.OutputParticipation(_value=_to_retrieved_edge(str(graph_model.age_name), row)) for row in result]
