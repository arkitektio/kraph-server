...
def protocol_events(
    ...
    """Fetch protocol events for a given category with optional filters, ordering, and pagination."""
    controller = context.get_controller()
    ...
    nodes = [
        retrieved.RetrievedNode(
            ...
            id=int(row["id"]),
            label=str(row.get("label", "ProtocolEvent")),
            properties=(row.get("n") or {}).get("properties", {}) if isinstance(row.get("n"), dict) else {},
        )
        for row in result
    ]

    return [types.ProtocolEvent(_value=node) for node in nodes]
